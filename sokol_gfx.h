...
#define _SG_LOG_ITEMS \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_STAGE_MISMATCH, "shader stage: image-sampler-pair stage doesn't match referenced image stage") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_SAMPLER_STAGE_MISMATCH, "shader stage: image-sampler-pair stage doesn't match referenced sampler stage") \
...
_SOKOL_PRIVATE sg_resource_state _sg_d3d11_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
    ...
    for (size_t ub_index = 0; ub_index < SG_MAX_UNIFORMBLOCK_BINDSLOTS; ub_index++) {
        ...
        const uint8_t slot = shd->d3d11.ub_register_b_n[ub_index];
        SOKOL_ASSERT(slot < _SG_D3D11_MAX_STAGE_UB_BINDINGS);
        if (stage == SG_SHADERSTAGE_VERTEX) {
            SOKOL_ASSERT(0 == shd->d3d11.vs_cbufs[slot]);
            shd->d3d11.vs_cbufs[slot] = cbuf;
        } else {
            SOKOL_ASSERT(0 == shd->d3d11.fs_cbufs[slot]);
            shd->d3d11.fs_cbufs[slot] = cbuf;
        }
    }
...
_SOKOL_PRIVATE void _sg_d3d11_discard_shader(_sg_shader_t* shd) {
    ...
    for (size_t i = 0; i < _SG_D3D11_MAX_STAGE_UB_BINDINGS; i++) {
        if (shd->d3d11.vs_cbufs[i]) {
            _sg_d3d11_Release(shd->d3d11.vs_cbufs[i]);
        }
        if (shd->d3d11.fs_cbufs[i]) {
            _sg_d3d11_Release(shd->d3d11.fs_cbufs[i]);
        }
    }
}
...
_SOKOL_PRIVATE bool _sg_validate_shader_desc(const sg_shader_desc* desc) {
        ...
        for (size_t ub_idx = 0; ub_idx < SG_MAX_UNIFORMBLOCK_BINDSLOTS; ub_idx++) {
            ...
            int num_uniforms = 0;
            for (size_t u_index = 0; u_index < SG_MAX_UB_MEMBERS; u_index++) {
                const sg_shader_uniform* u_desc = &ub->uniforms[u_index];
                if (u_desc->type != SG_UNIFORMTYPE_INVALID) {
